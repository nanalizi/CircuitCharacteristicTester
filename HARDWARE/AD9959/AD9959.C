#include "AD9959.H"
//https://blog.csdn.net/SUPPER__xxh/article/details/120981652
...
{ 
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0|GPIO_Pin_1|GPIO_Pin_9;//初始化管脚PB0.1.10			
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_2MHz;		 //IO口速度为2MHz
  ...
  IntReset();  //AD9959复位  
	...
	/*--------------------------------------------
WriteData_AD9959函数功能：控制器通过SPI向AD9959写数据
	寄存器地址    所含字节数    数据起始地址    是否更新IO寄存器
//CSR 通道选择寄存器
//FR1 功能寄存器1
//FR2 功能寄存器2 FR Frequency Range（频率范围）
//CFR 通道功能寄存器
//CTW0 通道频率转换字寄存器
//CPW0 通道相位转换字寄存器
//ACR 幅度控制寄存器
...
----------------------------------------------*/
  ...
  WriteData_AD9959(FR2_ADD,2,FR2_DATA,1);//写功能寄存器2
